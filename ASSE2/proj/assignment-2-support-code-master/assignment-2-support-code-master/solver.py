...
def main(arglist):
                        ...
                        if config2 is not False and not vcolli_check(config2) and not ecolli_check2(config1,config2):
                            ...
                            if points_distance(config2.points[current_end],target_point,"euclidean")<=1:
                                ...
                                if not vcolli_check(rev_config) and not vcolli_check(last_config):
                                    ...
                                    V.append(node2);
                                    V.append(node3);
                                    V_bridge.append(node2);
                                    V_bridge.append(node3);
    ...
    def connect_bridgeVetex():
        for i in range(len(V_bridge) - 1):
            for l in range(len(V_bridge) - i - 1):
                j = l + i + 1
                if distance(V_bridge[i].config, V_bridge[j].config) < 1:
                    if not ecolli_check2(V_bridge[i].config, V_bridge[j].config):
                        GraphNode.add_connection(V_bridge[i], V_bridge[j])
                        E.append((V_bridge[i], V_bridge[j]))
    ...
    k=0.1
    V=[]
    V_bridge = []  # only for bridge config
    E=[]
    V.append(init_node)
    V.append(goal_node)
    disjoint_sets={}
