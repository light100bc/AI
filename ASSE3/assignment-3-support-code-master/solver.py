...
class Solver:
    ...
    def get_mcts_policy(self, state):
        ...
        def set_mct_map(node):
            ...
            :return:
            """
            grid=clone_grid(node.map)
            mct_game_map.grid_data = grid
        ...
        def get_nextS_prob(node):
            ...
            grid = clone_grid(mct_game_map.grid_data)
            node = StateNode(grid, mct_game_map.player_x, mct_game_map.player_y,
        ...
        def simulate_Astrategy(node,n):
            ...
            """!!a good strategy can reduce the deepth of expand, it is a simple so the deepth is 1"""
            count=0
            while (count <= n+3):  # after n no result then give up
                count = count + 1
                if count <= 2:
                    set_mct_map(node)
                    # try shot first
        ...
        def one_simulate(node,action):
            ...
            n=2*(abs(node.player_x-self.F_x)+abs(node.player_y-self.F_y))
        ...
        grid=clone_grid(mct_game_map.grid_data)
        root = StateNode(grid,mct_game_map.player_x,mct_game_map.player_y,mct_game_map.player_heading)
        ...
        current=start
        count=0
        while(current-start<mct_game_map.time_limit and count<1): #mct deepth=1 because it is simple map, no need count>1
            count=count+1
            #initial
            """======================== selection+expand =============================================================="""
            ...
            """========================= backpropagate ==============================================================="""
            """( * directlly use result_list now, as deepth is 1, see conclusion at last of code)"""
            # backprobagate ( * directlly use result_list now, as deepth is 1, see conclusion at last of code)
            # the last one if not absorbing need all actions weight update !!!
            # the previous ones only one action weight need update
        ...
        max_action=''
        max_prob=0
        for action in result_list: #choose action with most n
            if max_prob<action[1]+action[3]:
                max_prob=action[1]+action[3]
                max_action=action[0]
    ...
    MCT usually have several level(deepth>1) in one iter
    But because this is simply map, I use deepth=1
    Only drawback is I need to choose coeff in simulator(rand walk +  invariant A*) to get a good performance
    
    This mean back propagate no need a function, just result_list is enough
    I choose win instrad of n to choose an action
    
    Finally, lack of time as got seminar this week,
    So did not extend deepth to >1, if deepth>1 algo will be more general
    But for testcases is enough (simulator is desinger according to the map, should have a more general one, but that means more deepth)  
    
    The only difficulty to make deepth>1 now is the __eq__ self.match.....may have other bugs not debug that part yet
    """
